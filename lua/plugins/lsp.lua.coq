return {
  ...
  },
  {
    ...
    config = function()
      ...
      local cap1 = coq.lsp_ensure_capabilities()
      require("mason").setup()
      local cap2 = {
        capabilities = {
        }
      ...
      local maps = { cap1 } or { cap2, cap1 }
      local new =
          vim.tbl_deep_extend("force", vim.empty_dict(), unpack(maps))

      for _, server_name in pairs(mslsp.get_installed_servers()) do
        --  Use server_name directly to get the config function.
        local config = {
          capabilities = new, --  Pass the capabilities here
        }
